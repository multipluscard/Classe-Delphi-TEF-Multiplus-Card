...
type
  ...
  Tfrmwebtef = class(TForm)
    ...
    procedure ActivateTEF(Sender: TObject);
...
begin
   if edtdado.Text<>'' then
      ...
      begin
         beep;
         pnalerta.Caption      := 'Valor inválido !';
...
procedure Tfrmwebtef.ActivateTEF(Sender: TObject);
begin
   ...
   if not entrou then
      begin
         entrou             := true;
         //---------------------------------------------------------------------
         logomp.Picture.LoadFromFile(GetCurrentDir+'\icones\tef_mplpay.bmp');    // Multiplus
         titulo.Caption := 'TEF Multipluscard';    // Multiplus Pay
         if frmwebtef.TituloJanela<>'' then
            titulo.Caption := frmwebtef.TituloJanela;
         //---------------------------------------------------------------------
         frmwebtef.Repaint;
      end;
   //---------------------------------------------------------------------------
